...
section .data
    ...
    pauseMsg db 0x0A,"Q to quit... hit space again to continue",0x0A,"Use W,A,S,D to move",0x00
    portal db "0",0x00
    portalerror db "PORTAL ERROR: VALID FORMAT REQUIRES 0 or 2 PORTALS!",0x0A,0x00
...
_GameTick:
    ...
    _skipWrapTailIndex:    
       ...
       ;Check to see if the assemblipede hits a wall
       cmp byte [eax],'-'
       je _exit
       cmp byte [eax],'|'
       je _exit
...
_LoadLevelLoop:
    ...
    ;Check to see if it found a portal
    cmp eax, 0
    jne _foundPortal
...
_replaceportal1:
	mov dword [portal1], eax
	;inc dword [portalcount] 
	jmp _portalreturn

_replaceportal2:
	mov dword [portal2], eax
	;inc dword [portalcount] 
	jmp _portalreturn
